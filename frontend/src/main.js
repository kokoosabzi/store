...
const App = {
  setup() {
    ...
    const nav = [{ id: 'dashboard', label: 'داشبورد' }, { id: 'products', label: 'کالاها' }, { id: 'inventory', label: 'موجودی' }]
  ...
  template: `
    <div class="app-shell">
      <aside class="sidebar"><div class="brand"><span>LC</span><strong>هسته بازرگانی</strong><small>محلی و آفلاین</small></div>
        <nav><button v-for="item in nav" :key="item.id" :class="{active: activeView === item.id}" @click="activeView = item.id">{{ item.label }}</button></nav>
      ...
      <main><header><div><p class="eyebrow">LOCAL COMMERCE CORE</p><h1>{{ title }}</h1></div><button class="refresh" @click="refresh" :disabled="loading">{{ loading ? 'در حال بروزرسانی…' : 'بروزرسانی' }}</button></header>
        ...
        <template v-else><section class="panel"><h2>وضعیت موجودی</h2><table><thead><tr><th>کالا</th><th>موجودی فعلی</th><th>حداقل موجودی</th></tr></thead><tbody><tr v-for="item in products" :key="item.id"><td>{{ item.name }}</td><td>{{ item.current_stock }}</td><td>{{ item.min_stock }}</td></tr></tbody></table></section></template>
      </main>
    </div>`
}
